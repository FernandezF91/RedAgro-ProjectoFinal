...
class PantallaPrincipalconsumidores extends Component {
    ...
    componentDidMount() {
        ...
        fetch("redAgro/obtener_productos", {
            method: "GET",
            headers: {
        ...
        const item = [
            <MDBRow>
                <MDBCol className="scrollMenuCategorias">
                    {
                        verduras.map((v, index) => {
                            return <MDBRow key={index} id="subTipos" className="cursorManito" onClick={() => this.handleNuevaBusqueda(v.tipo)}>
                                {v.tipo}
                            </MDBRow>
                        })
                    }
    ...
    obtenerNotificaciones() {
        var _this = this;
        var path = "redAgro/AlertaNotificaciones/obtenerAlertasByUsuario?id_usuario=" + localStorage.getItem('myLocalStorageIdConsumidor');
        fetch(path, {
            method: "GET",
            headers: {
                'Content-type': 'application/json;charset=UTF-8',

...
class ListadoReservas extends Component {
    ...
    cargarListadoDeEstados() {
        var path = "http://"+window.$ip+":3000/redAgro/obtenerEstadosReserva";
        fetch(path)
    ...
    componentDidMount() {
        ...
        var path_usuario = "http://"+window.$ip+":3000/redAgro/get_reservas_usuario?id=" + _this.state.id;
        fetch(path_usuario)
            .catch(err => console.error(err))
            .then(response => {
                if (response.status === 200) {
    ...
    actualizarEstadoReserva() {
        ...
        var path = "http://"+window.$ip+":3000/redAgro/actualizarEstadoReserva?id_reserva=" + _this.state.idReservaActualizar + "&id_estado=" + _this.state.estadoSeleccionado.value + "&rol=" + this.props.usuario.rol;
        fetch(path, {
            method: "PUT"
        })
    ...
    guardarCalificacion() {
        ...
        var path = "http://"+window.$ip+":3000/redAgro/guardarCalificacion?reserva_id=" + _this.state.idReservaActualizar;
        fetch(path, {
            method: "POST",
            headers: {

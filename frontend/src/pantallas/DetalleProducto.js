...
class DetalleBusqueda extends Component {
    constructor(props) {
        ...
        this.sumarProducto = this.sumarProducto.bind(this);
            ...
            <Container className="containerPrincipal">
                <Row>
                    <Col className="imagenProducto" sm={8}>
                        <Carousel >
                            {
                                this.props.producto.imagenes.map((imagen, index) => (
                    ...
                    <Col className="detalleDelProducto" sm={4}>
                        <br />
                        <h6 className="grey-text">{this.props.producto.categoria}</h6>
                        <h3 className="tituloProducto">{this.props.producto.titulo}</h3>
                        <h3 className="precioProducto">
                            <NumberFormat value={this.props.producto.precio} displayType={'text'} thousandSeparator={"."} decimalSeparator={","} prefix="$ " decimalScale={2} fixedDecimalScale={true} /></h3>
                        x {this.props.producto.tipoDeUnidad}

                        <div className="descripcionProducto">
                            Producido por:
                        <h6>{this.props.producto.productor.razon_social}</h6>
                        </div>

                        <div className="descripcionProducto">
                            Descripción del producto:
                        <h6>{this.props.producto.descripcion}</h6>
                        </div>
                        <div className="descripcionProducto">
                            Tipo de producción:
                        <h6>{this.props.producto.tipoDeProduccion}</h6>
                        </div>
                        {
                            this.props.producto.fechaDeVencimiento !== undefined ?
                                <div className="descripcionProducto">
                                    Fecha de vencimiento: <h6>{moment(this.props.producto.fechaDeVencimiento, 'YYYY-MM-DD').format("DD/MM/YYYY")}</h6>
                                </div>
                                : ''
                        }
                        <div className="botonesDetalle">
                            ...
                            <span className="float-center">{this.props.producto.cantidad}</span>

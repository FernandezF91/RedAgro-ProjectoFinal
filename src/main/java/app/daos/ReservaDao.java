...
public interface ReservaDao extends JpaRepository<EntidadReserva, Long> {
	...
	@Query(value = "SELECT * FROM Reserva WHERE consumidor_id = ?1 ORDER BY Fecha DESC", nativeQuery = true)
	List<EntidadReserva> obtenerReservasByConsumidor(long id);

	@Query(value = "SELECT * FROM Reserva WHERE productor_id = ?1 ORDER BY Fecha DESC", nativeQuery = true)

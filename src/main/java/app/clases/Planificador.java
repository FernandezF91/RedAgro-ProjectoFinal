...
public class Planificador {
	...
	public List<String> obtenerResultados() {
		...
		net.readFile("C:/Bayes/BayesCulturaVerSde.xdsl");
	...
	public void run(List<String> lista) {
		...
		int s = createCptNode(net, "categoria", "Categoria de los productos", new String[] { "Frutas", "Verduras" }, 60,
				40);

		int f = createCptNode(net, "periodo", "Período de producción anual",
				new String[] { "Verano", "Primavera", "Invierno" }, 110, 140);
		...
		System.out.println("Tutorial1 complete: Network written to tutorial1.xdsl");
	...
	public void tutorial2() {
		...
		System.out.println("Removing evidence from Economy, keeping Forecast=Poor.");
	...
	private static void printPosteriors(Network net, int nodeHandle) {
		...
		if (net.isEvidence(nodeHandle)) {
			...
			for (int i = 0; i < posteriors.length; i++) {
				System.out.printf("P(%s=%s)=%f\n", nodeId, net.getOutcomeId(nodeHandle, i), posteriors[i]);
			}
		}
	}
